...
function Install_initial_system_setting()
    ...
    for i = 0 , 3 do
        TriggerX(Force1, {Deaths(i, AtLeast, 1, 0)}, {
            ...
            DisplayText("\x12\x1F【"..ColorArray[i+1].." ["..(i+1).." Player]\x04의 \x03P\x04rivate \x06M\x04arine 이 \x06사망\x04하였습니다.\x1F】", 4);

        },preserved); -- 일마

        TriggerX(Force1, {Deaths(i, AtLeast, 1, 20)}, {
            SetScore(i, Add, 2, Custom),
            DisplayText("\x12\x1F【"..ColorArray[i+1].." ["..(i+1).." Player]\x04의 \x03H\x04ero \x06M\x04arine 이 \x06사망\x04하였습니다.\x1F】", 4);

        }, preserved); -- 영마

        TriggerX(Force1, {Deaths(i, AtLeast, 1, 1)}, {
            SetScore(i, Add, 4, Custom),
            DisplayText("\x12\x1F【"..ColorArray[i+1].." ["..(i+1).." Player]\x04의 \x03S\x04pecial \x06M\x04arine 이 \x06사망\x04하였습니다.\x1F】", 4);
...
players = {P6},
    ...
    Trigger { -- 노래시간 감소
        ...
        actions = {
            SetDeaths(Force1, Subtract, 1, 221); -- 순대 브금 타이머
            PreserveTrigger();
        },
    }
...
function Install_NormalGunPlotShape()
    ...
    function HatcheryGunplot2( Hatname, Deathvar)
        ...
        CSPlot(Eftstar, P6, 33, stargatename, nil, 1, 32, P6,{CommandLeastAt(167, stargatename), Deaths(P10, AtLeast, StargateEftTime[4] * SDspeed, Deathvar)})
        CSPlot(Eftstar, P7, 60, stargatename, nil, 1, 32, P7,{CommandLeastAt(167, stargatename), Deaths(P10, AtLeast, StargateGenTime[26] * SDspeed, Deathvar)})
        for i = 27,27 do
...
function Install_SpecialGunPlotShape()
    -- Cerebrate
    TriggerX(FP, {CommandLeastAt(151, "celebrate1")}, {SetDeaths(P10, Add, 1, 60)},preserved);
    for i = 1, 24 do
        Circular = CSMakeLineX(1,60,0+15*i,7,1) -- Circle
        CSPlot(Circular,P6,55,"celebrate1",nil,1,48,P7,{CommandLeastAt(151,"celebrate1"),Deaths(P10,Exactly,4*i,60)})
        CSPlot(Circular,P6,56,"celebrate1",nil,1,48,P7,{CommandLeastAt(151,"celebrate1"),Deaths(P10,Exactly,800+4*i,60)})
    end
    CSPlot(CX2,P6,51,"celebrate1",nil,1,48,P7,{CommandLeastAt(151,"celebrate1"),Deaths(P10,Exactly,96,60)}) --외접원
    CSPlot(CX2,P6,46,"celebrate1",nil,1,48,P7,{CommandLeastAt(151,"celebrate1"),Deaths(P10,Exactly,896,60)}) --외접원

    TriggerX(FP, {CommandLeastAt(151, "celebrate1"),Deaths(P10, AtLeast, 100, 60)}, {Order("Any unit", Force2, "celebrate1", Attack, "HealZone")})

    for j = 1, 24 do
        GLj = CSMakeLineX(1,60,360-15*j,11,1) -- 원그리기 2
        CSPlot(GLj,P6,89,"celebrate1",nil,1,64,P7,{CommandLeastAt(151,"celebrate1"),Deaths(P10,Exactly,400+4*j,60)})
        
        CSPlot(GLj,P6,88,"celebrate1",nil,1,64,P7,{CommandLeastAt(151,"celebrate1"),Deaths(P10,Exactly,1200+4*j,60)})
    end
        
        CSPlot(CX2,P6,65,"celebrate1",nil,1,48,P7,{CommandLeastAt(151,"celebrate1"),Deaths(P10,Exactly,496,60)})
        CSPlot(CX2,P6,17,"celebrate1",nil,1,48,P7,{CommandLeastAt(151,"celebrate1"),Deaths(P10,Exactly,1296,60)})
